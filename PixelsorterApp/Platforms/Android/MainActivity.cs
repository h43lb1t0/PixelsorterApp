            ...
            if (intent?.Action == Android.Content.Intent.ActionSend && intent.Type != null)
            {
                if (intent.Type.StartsWith("image/"))
                {
                    if (intent.Type == "image/x-adobe-dng")
                    {
                        ...
                        Android.Widget.Toast.MakeText(Android.App.Application.Context, PixelsorterApp.Resources.Languages.AppStrings.common_dngErrorMessage, Android.Widget.ToastLength.Long)?.Show();
